...
#pragma implementation "instrmix/suif_pass.h"
#endif
...
extern "C" void
init_instrmix(SuifEnv *suif_env)
{
    ...
    mSubSystem->register_module(new InstrMixSuifPass(suif_env));
...
InstrMixSuifPass::InstrMixSuifPass(SuifEnv *suif_env, const IdString& name)
    : PipelinablePass(suif_env, name)
{
    the_suif_env = suif_env;	// bind suif_env into our global environment
}

InstrMixSuifPass::~InstrMixSuifPass()
{
    // empty
}

void
InstrMixSuifPass::initialize()
{
...
bool
InstrMixSuifPass::parse_command_line(TokenStream *command_line_stream)
{
    // Set flag defaults
...
void
InstrMixSuifPass::do_file_set_block(FileSetBlock *fsb)
{
...
void
InstrMixSuifPass::do_file_block(FileBlock *fb)
{
...
void
InstrMixSuifPass::do_procedure_definition(ProcedureDefinition *pd)
{
    ...
    focus(pd);
    instrmix.do_opt_unit(pd);
    defocus(pd);
}

void
InstrMixSuifPass::finalize()
{
    instrmix.finalize();
}
